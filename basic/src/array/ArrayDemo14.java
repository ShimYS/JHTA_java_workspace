...
public class ArrayDemo14 {
	...
	public static void main(String[] args) {
		...
		for(int i=0; i<secretNumbers.length; i++) {
			...
			for(int j=0; j<i; j++) {
				...
				if(secretNumbers[j] == randomNumber) {
					...
					//숫자를 비교하는 for문을 탈출한다.
					isExist = true;
					break; //똑같은게 발견되는 즉시 For문 탈출
				}
			}
			//isExist가 false면 중복없음, isExitst가 true면 중복이 발생함
			if(isExist) {
				//이번에 뽑은 난수가 중복이 발생했으므로 다시 이번회차를 뽑기 위해 i를 1감소시킴
				i--;
			} else {
				//중복이 없으므로 난수를 배열에 저장
		...
		while(true) {
			...
			System.out.println("입력값: " + inputNumbers[0] + " " + inputNumbers[1] + " " + inputNumbers[2]);
